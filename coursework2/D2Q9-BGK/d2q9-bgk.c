...
int synchronise(const t_param params, t_speed* cells, int size, int rank);
...
int timestep(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles, int size, int rank)
{
  ...
  synchronise(params, cells, size, rank);
...
int synchronise(const t_param params, t_speed* cells, int size, int rank)
{
    ...
    MPI_Status status;
    MPI_Sendrecv(&(cells[params.ny*params.nx]), params.nx * NSPEEDS, MPI_INT, right, 0, cells, params.nx * NSPEEDS, MPI_INT, left, 0, MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&(cells[params.nx]), params.nx * NSPEEDS * NSPEEDS, MPI_INT, left, 0, &(cells[(params.ny + 1)*params.nx]), params.nx * NSPEEDS, MPI_INT, right, 0, MPI_COMM_WORLD, &status);
    return EXIT_SUCCESS;
}

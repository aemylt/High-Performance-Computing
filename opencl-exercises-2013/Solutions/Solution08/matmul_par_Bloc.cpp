...
//           product matrix, C.  Each work-item computes a row of C.
//           Reduce traffic to global memory by copying a row of A
//           into private memory.  Share a column of the B matrix 
//           with the work-items in a work-group.
//
...
int main(void)
{
    ...
    try
    {
       ...
       cl::Program program(context, util::loadProgram("matmul_Bloc.cl"));
       try
       {
           program.build();
       }
       catch (cl::Error error)
       {
        ...
        auto mmul = cl::make_kernel<cl::Buffer, cl::Buffer, cl::Buffer, 
        ...
        // Work-items compute a row of C.   64 work-items per work-group. 

        cl::LocalSpaceArg B_col = cl::Local(sizeof(float) * N);
